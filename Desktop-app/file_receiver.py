...
class FileReceiver(QThread):
    ...
    def run(self):
        ...
        self.client_socket.connect((self.device_info.get("ip"), RECEIVER_PORT))
        ## Wait for a start signal
        while True:
            start_flag = self.client_socket.recv(8).decode()
            if start_flag == 'START':
                break
        self.receive_files()
...
class CreateUIForReceiver(QWidget):
    ...
    def initUI(self):
        ...
        sender = self.device_info.get("device_name", "Unknown Device")
        self.label = QLabel("Connected with " + sender, self)
